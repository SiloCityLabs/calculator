...
My journey to an appless life starts with PWAs. The Play Store Google Calculator sits at **~10.32 MB** on my phone. This PWA ships the same kind of experience in about **129 KB** (~91 KB transferred with gzip) — roughly **78× smaller**, fully offline once installed, and no store required.
...
| This PWA (all shipped files) | **~129 KB** (0.126 MB) |
| Typical transfer (gzip text + icons) | **~91 KB** |
